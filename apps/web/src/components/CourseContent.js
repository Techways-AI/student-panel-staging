      ...
      if (currentTopic) {
        const markNotesCompleted = async () => {
          try {
            ...
            const findTaskResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://127.0.0.1:8000'}/api/study-plan/task/find-by-topic?subject=${encodeURIComponent(subjectName)}&topic=${encodeURIComponent(topicName)}`, {
              method: 'GET',
              headers: {
            ...
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://127.0.0.1:8000'}/api/study-plan/task/mark-notes-completed`, {
              method: 'POST',
              headers: {
                'Authorization': `Bearer ${token}`,
                'Content-Type': 'application/json',
